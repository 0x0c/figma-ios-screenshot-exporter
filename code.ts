...
// Export targets are the currently selected layers as-is (not filtered by type).
type ExportNode = SceneNode;
...
function resolveLabel(width: number, height: number, scale: number): string | null {
...
function getTargetNodes(): ExportNode[] {
  const sel = figma.currentPage.selection;
  const source: ReadonlyArray<SceneNode> =
    sel.length > 0 ? sel : figma.currentPage.children;
  return source.filter(
    (n): n is ExportNode => typeof n.exportAsync === "function"
  );
}

// ---- Reorder into reading order by position (row clustering) ----------
function orderByCanvasPosition(nodes: ExportNode[], rowTolerance: number): ExportNode[] {
  ...
  for (const node of sorted) {
    ...
    if (!placed) rows.push({ centerY, count: 1, nodes: [node] });
  }
  rows.sort((a, b) => a.centerY - b.centerY);
  rows.forEach((row) => row.nodes.sort((a, b) => a.x - b.x));
  const ordered: ExportNode[] = [];
  for (const row of rows) ordered.push(...row.nodes);
  return ordered;
}
...
function buildName(
  ...
  if (opts.includeName) base += `${sep}${sanitize(originalName)}`;
  if (opts.locale) base = `${sanitize(opts.locale)}${sep}${base}`; // prefix with the locale
  return base;
}
...
figma.ui.onmessage = async (msg: UIMessage) => {
  ...
  const warnings: { name: string; w: number; h: number }[] = [];
  const seqByLabel: Record<string, number> = {}; // per-family sequence counter
